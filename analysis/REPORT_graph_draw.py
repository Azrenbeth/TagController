...
def plot_progress(ax, filename, colour, label, start):
    # with np.load(filename) as end_times:
    end_times = np.load(filename)
    times, counts = times_and_counts(end_times[start:])
...
############
# DONE
plot_all("write_every_leaf", start=200)
plot_all("write_every_root", start=200)
plot_all("write_every_leaf_line", limits=1000, start=200)
plot_all("write_every_root_line", limits=1000, start=200)
...
plot_all("overtaking_leaf_two", limits=10000, simulation_names=["not_ooo", "final"])
plot_all("overtaking_leaf_ten", limits=10000, simulation_names=["not_ooo", "final"])
# plot_all("overtaking_leaf_single", limits=10000)
# plot_all("overtaking_leaf_ten", limits=10000)
